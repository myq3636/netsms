...
public class ChangeRedisAck extends SystemPdu {
    private static SystemLogger log = SystemLogger.getSystemLogger(ChangeRedisAck.class);
    private int responseCode = -1;
    public ChangeRedisAck(SystemPdu req) {
        if (header == null) {
            header = new SystemPduHeader();
        }
        header.setCommandId(this.COMMAND_CHANGEREDIS_ACK);
        header.setSequenceNumber(req.getSequenceNumber());
    }
	public ChangeRedisAck() {
	        if (header == null) {
	            header = new SystemPduHeader();
	        }
	        header.setCommandId(this.COMMAND_CHANGEREDIS_ACK);
	...
	public String toString() {
        return new StringBuffer("CHANGEREDIS_ACK uuid:").append(uuid).append(",")
