 ...
 * @version 1.0.0
 */
public class ApplyInThrottleQuotaAck extends SystemPdu {
	
	private static SystemLogger log = SystemLogger.getSystemLogger(ApplyInThrottleQuotaAck.class);
	 ...
	 * 0~100
	 */
	private int sysLoadPercent = 0;
	
	public ApplyInThrottleQuotaAck() {
        if (header == null) {
            header = new SystemPduHeader();
        }
        header.setCommandId(SystemPdu.COMMAND_APPLY_IN_THROTTLE_QUOTA_ACK);
    }
	...
	public ApplyInThrottleQuotaAck(SystemPdu req) {
        if (header == null) {
            header = new SystemPduHeader();
        }
        header.setCommandId(SystemPdu.COMMAND_APPLY_IN_THROTTLE_QUOTA_ACK);
	...
	public String toString() {
		return new StringBuffer("APPLY_IN_THROTTLE_QUOTA_ACK uuid:").append(uuid).append(",")
        .append("timestamp:").append(timestamp)
        .append(",ssid:").append(ssid)
        .append(",sysLoadPercent:").append(sysLoadPercent)
        .toString();
	}
	    ...
	    while (buffer.length() > 0) {
	        ...
	        switch (tag) {
	            ...
	            case FIELD_SSID:
	                ...
	                break;
	            case FIELD_SYS_LOAD_PERCENT:
	            	sysLoadPercent = buffer.removeBytesAsInt(length);
	                break;
	            default:
	...
	public TcpByteBuffer pduCommandToByteBuffer() throws NotEnoughDataInByteBufferException, UnsupportedEncodingException {
	    ...
	    appendParameterToBuffer(buffer, FIELD_SYS_LOAD_PERCENT, sysLoadPercent,1);
	    return buffer;
	}
	...
	public int getSysLoadPercent() {
		return sysLoadPercent;
	}

	public void setSysLoadPercent(int sysLoadPercent) {
		this.sysLoadPercent = sysLoadPercent;
	}
