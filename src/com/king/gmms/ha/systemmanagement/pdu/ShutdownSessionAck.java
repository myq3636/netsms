...
public class ShutdownSessionAck extends SystemPdu {
    private static SystemLogger log = SystemLogger.getSystemLogger(ShutdownSessionAck.class);
    private int responseCode = -1;
	public ShutdownSessionAck(SystemPdu req) {
	        if (header == null) {
	            header = new SystemPduHeader();
	        }
	        header.setCommandId(this.COMMAND_SHUTDOWN_SESSION_ACK);
	        header.setSequenceNumber(req.getSequenceNumber());
	 }
	public ShutdownSessionAck() {
        if (header == null) {
            header = new SystemPduHeader();
        }
        header.setCommandId(this.COMMAND_SHUTDOWN_SESSION_ACK);
	...
	public String toString() {
        return new StringBuffer("SHUTDOWN_SESSION_ACK uuid:").append(uuid).append(",")
