...
public class ConnectionStatusNotificationAck extends SystemPdu {
    private static SystemLogger log = SystemLogger.getSystemLogger(ConnectionStatusNotificationAck.class);
    private int responseCode = -1;
    public ConnectionStatusNotificationAck(SystemPdu req) {
        if (header == null) {
            header = new SystemPduHeader();
        }
        header.setCommandId(this.COMMAND_CONNECTION_STATUS_NOTIFICATION_ACK);
        header.setSequenceNumber(req.getSequenceNumber());
    }
	public ConnectionStatusNotificationAck() {
	        if (header == null) {
	            header = new SystemPduHeader();
	        }
	        header.setCommandId(this.COMMAND_CONNECTION_STATUS_NOTIFICATION_ACK);
	...
	public String toString() {
        return new StringBuffer("CONNECTION_STATUS_NOTIFICATION_ACK ").append(super.toString())
