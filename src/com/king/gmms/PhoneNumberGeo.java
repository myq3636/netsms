...
public class PhoneNumberGeo {
  ...
  private static final int DATA_FILE_LENGTH_HINT = 4747505;
  private static final String PHONE_DAT_FILE_PATH = "conf/phone.dat";
  ...
  private static synchronized void initData(String a2pHome) {
    if (dataByteArray == null) {
      ByteArrayOutputStream byteData = new ByteArrayOutputStream(DATA_FILE_LENGTH_HINT);
      byte[] buffer = new byte[1024];

      int readBytesLength;
      try (InputStream inputStream = new FileInputStream(a2pHome+PHONE_DAT_FILE_PATH)) {
        while ((readBytesLength = inputStream.read(buffer)) != -1) {
          byteData.write(buffer, 0, readBytesLength);
        }
      } catch (Exception e) {
        System.err.println("Can't find phone.dat in classpath: " + a2pHome+PHONE_DAT_FILE_PATH);
        e.printStackTrace();
  ...
  public PhoneNumberGeo(String a2phome) {
    initData(a2phome);
  ...
  public static void main(String[] args) {
	  System.out.println(PhoneNumberGeo.class.getResource("/"));
    PhoneNumberGeo phoneNumberGeo = new PhoneNumberGeo("");
    System.out.println(phoneNumberGeo.lookup("19848011584"));
