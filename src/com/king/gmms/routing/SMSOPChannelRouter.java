...
/**
 * <p>
 * Title: IOSMSChannelRouter
 * </p>
 * <p>
 * Description: ChannelRoute of IOSMS service, route the messages to channels
 * </p>
 * <p>
 ...
 * @author: Neal
 */
public class SMSOPChannelRouter extends ChannelRouter {
	// private static Logger log = Logger.getLogger(IOSMSChannelRouter.class);
	private static SystemLogger log = SystemLogger
			.getSystemLogger(SMSOPChannelRouter.class);

	/**
	 * Constructor
	 */
	public SMSOPChannelRouter() {
	...
	public boolean dispatch(GmmsMessage msg) {
		...
		if (GmmsMessage.MSG_TYPE_SUBMIT.equalsIgnoreCase(msg.getMessageType())
			...
			if (msg.getRSsID() > 0) {
				...
				//TODO
				rSsid = ctm.getCustomerOPSenderRoutingRelay(ossid, msg, msg.getRoperator());
				if (rSsid>0) {
					log.info(msg,"get Rssid: {} by OP sender routingRelay",rSsid);
				}else{
					rSsid = ctm.getCustomerOPContentRoutingRelay(ossid, msg, msg.getRoperator());
					if (rSsid>0) {
						log.info(msg,"get Rssid: {} by OP content keyword routingRelay",rSsid);
					}else {
						rSsid = ctm.getCustomerOPRoutingRelay(ossid, msg, msg.getRoperator());
						if(log.isInfoEnabled()){
							log.info(msg,"get Rssid: {} by OP routingRelay",rSsid);
						}
					}
				}
