 ...
 * @author: Neal
 */
public class IOSMSChannelRouter extends ChannelRouter {
	// private static Logger log = Logger.getLogger(IOSMSChannelRouter.class);
	private static SystemLogger log = SystemLogger
			.getSystemLogger(IOSMSChannelRouter.class);

	/**
	 * Constructor
	 */
	public IOSMSChannelRouter() {
	...
	public boolean dispatch(GmmsMessage msg) {
		...
		if (GmmsMessage.MSG_TYPE_SUBMIT.equalsIgnoreCase(msg.getMessageType())
			...
			if (msg.getRSsID() > 0) {
				...
				//TODO
				rSsid = ctm.getCustomerSenderRoutingRelay(ossid, msg);
				if (rSsid>0) {
					log.info(msg,"get Rssid: {} by sender routingRelay",rSsid);
				}else{
					rSsid = ctm.getCustomerContentKeywordRoutingRelay(ossid, msg);
					if (rSsid>0) {
						log.info(msg,"get Rssid: {} by content keyword routingRelay",rSsid);
					}else {
						rSsid = ctm.getCustomerRoutingRelay(ossid, msg);
						if(log.isInfoEnabled()){
							log.info(msg,"get Rssid: {} by routingRelay",rSsid);
						}
					}
				}
