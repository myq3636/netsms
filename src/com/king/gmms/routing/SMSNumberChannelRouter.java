 ...
 * @author: Neal
 */
public class SMSNumberChannelRouter extends ChannelRouter {
	// private static Logger log = Logger.getLogger(IOSMSChannelRouter.class);
	private static SystemLogger log = SystemLogger
			.getSystemLogger(SMSNumberChannelRouter.class);

	/**
	 * Constructor
	 */
	public SMSNumberChannelRouter() {
	...
	public boolean dispatch(GmmsMessage msg) {
		...
		if (GmmsMessage.MSG_TYPE_SUBMIT.equalsIgnoreCase(msg.getMessageType())
			...
			if (msg.getRSsID() > 0) {
				...
				//TODO
				rSsid = ctm.getCustomerNumberRoutingRelay(ossid, msg);
				if (rSsid>0) {
					log.info(msg,"get Rssid: {} by number routingRelay",rSsid);
				}
