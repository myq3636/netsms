...
public class UdpByteBuffer {
    ...
    public UdpByteBuffer(byte[] newBuffer) {
        buffer = newBuffer;
    }

    public UdpByteBuffer(UdpByteBuffer bb){
        this.buffer = bb.getBuffer();
    }

    public void appendBytes(UdpByteBuffer bytes)
        throws NotEnoughDataInByteBufferException {
        appendBytes(bytes,bytes.length());
    }

    public void appendBytes(UdpByteBuffer bytes, int count)
        throws NotEnoughDataInByteBufferException {
        if (count > 0) {
            if (bytes == null) {
    ...
    public void appendIntAs1Byte(int value)
        ...
        appendBytes(new UdpByteBuffer(buffer), 1);
    }
    ...
    public void appendIntAs2Byte(int value)
        ...
        appendBytes(new UdpByteBuffer(buffer), 2);
    }
    ...
    public void appendLongAs4Bytes(long value)
        ...
        appendBytes(new UdpByteBuffer(buffer), 4);
    }
    ...
    public void appendString(String value) throws
        ...
        appendString(value, value.getBytes(CHARSET).length);
    }
    ...
    public void appendString(String value, int count) throws
        ...
        if (buf.length < count) {
            appendBytes(new UdpByteBuffer(buf), buf.length);

            byte[] buf1 = new byte[count - buf.length];
            for (int i = 0; i < buf1.length; i++) {
                buf1[i] = 0;
            }
            appendBytes(new UdpByteBuffer(buf1), buf1.length);
        } else {
            appendBytes(new UdpByteBuffer(buf), count);
        }
    }
    ...
    public UdpByteBuffer readBytes(int count)
        throws NotEnoughDataInByteBufferException {
        int len = length();
        UdpByteBuffer result = null;
        if (count > 0) {
            if (len >= count) {
                byte[] resBuf = new byte[count];
                System.arraycopy(buffer, 0, resBuf, 0, count);
                result = new UdpByteBuffer(resBuf);
                return result;
            }
            else {
    ...
    public UdpByteBuffer removeBytes(int count)
        throws NotEnoughDataInByteBufferException {
        UdpByteBuffer result = readBytes(count);
    ...
    public String removeString(int size) throws
            NotEnoughDataInByteBufferException, UnsupportedEncodingException {
        return removeString(size, CHARSET);
    }
