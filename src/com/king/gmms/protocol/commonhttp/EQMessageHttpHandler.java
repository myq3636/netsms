...
public class EQMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger.getSystemLogger(EQMessageHttpHandler.class);
	public EQMessageHttpHandler(HttpInterface hie){		
		super(hie);	
	}
	/**
	...
	public String makeRequest(GmmsMessage message,String urlEncoding, A2PCustomerInfo cst)throws UnsupportedEncodingException {
		...
		String senderWithPlus = sender;
		if (!sender.startsWith("+")) {
			senderWithPlus = "+"+senderWithPlus;
		}
		String recipientWithPlus = recipient;
		if (!recipient.startsWith("+")) {
			recipientWithPlus = "+"+recipientWithPlus;
		}
		...
		for(HttpParam param:parameters){
			...
			if("recipientAddress".equalsIgnoreCase(pval)){
				postData.append("&").append(param.getOppsiteParam()).append("=").append(urlEncode(recipientWithPlus,urlEncoding));//recipient
			}else if("senderAddress".equalsIgnoreCase(pval)){			
				postData.append("&").append(param.getOppsiteParam()).append("=").append(urlEncode(senderWithPlus, urlEncoding));//message
	...
	public void parseResponse(GmmsMessage msg, String resp) {
			try {
	            ...
	            if (respStr!= null) {
	            	...
	            	msg_status_text = (String)json.get("Status");
	            	outMsgID = (String)json.get("MessageID");
				}	            	         	            
	            
				HttpStatus status = new HttpStatus(msg_status_text,msg_status_text);
