...
public class ColonFormatHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(ColonFormatHttpHandler.class);
	private static final String LINE_WRAP_CHAR = "\r\n";
	private static final String COLON_CHAR = ":";

	public ColonFormatHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	private String generateSubmitResponse(HttpStatus hs, GmmsMessage msg,
		...
		for (HttpParam hp : respList) {
			...
			if ("StatusText".equalsIgnoreCase(param)) {
				content.append(oparam).append(COLON_CHAR).append(hs.getText())
						.append(LINE_WRAP_CHAR);
			} else if ("StatusCode".equalsIgnoreCase(param)) {
				content.append(oparam).append(COLON_CHAR).append(hs.getCode())
						.append(LINE_WRAP_CHAR);
			} else {
				...
				content.append(oparam).append(COLON_CHAR).append(respValue)
						.append(LINE_WRAP_CHAR);
			}
			respValue = "";
		}
	...
	private String makeDRResponse(HttpStatus hs, GmmsMessage message,
		...
		for (HttpParam hp : respList) {
			...
			if ("StatusCode".equalsIgnoreCase(param)) {
				content.append(hp.getOppsiteParam()).append(COLON_CHAR).append(
						hs.getCode()).append(LINE_WRAP_CHAR);
			} else if ("StatusText".equalsIgnoreCase(param)) {
				content.append(hp.getOppsiteParam()).append(COLON_CHAR).append(
						hs.getText()).append(LINE_WRAP_CHAR);
			} else {
				respValue = HttpUtils.getParameter(hp, message, cst);
				content.append(hp.getOppsiteParam() + COLON_CHAR + respValue
						+ LINE_WRAP_CHAR);
			}
	...
	private HttpStatus parseSubmitRequest(GmmsMessage message,
		...
		try {
			...
			log
					.warn(
							"Error occur when processing throttling control in ColonFormatHttpHandler.parseSubmitRequest",
							e);
		}
	...
	public void parseResponse(GmmsMessage message, String resp) {
		...
		if (message.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT)) {
			...
			log.info(message, "Receive DR response and the inMsgId is "
					+ inMsgId);
	...
	private Map<String, String> genResponseMap(GmmsMessage message,
		...
		for (String line : lines) {
			...
			int nIndex = line.indexOf(COLON_CHAR);
			if (nIndex == -1) {
				log.error("No '{}' to split line:", COLON_CHAR, line);
				continue;
			}
			resp[0] = line.substring(0, nIndex).trim();
			resp[1] = line.substring(nIndex + 1).trim();
			map.put(resp[0], resp[1]);
		}
		...
		if (message.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT)) {
			...
			if (hs == null) {
				...
				if(log.isInfoEnabled()){
					log.info(message, "Get HttpStatus for DR response:{}",hs);
				}
			}
		}
		return map;
