...
public class ExtendibleDelimiterHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger.getSystemLogger(ExtendibleDelimiterHttpHandler.class);
	
	public ExtendibleDelimiterHttpHandler(HttpInterface hie){
		super(hie);
	}
	/**
	 * make request
	 */
	public String makeRequest(GmmsMessage message, String urlEncoding, A2PCustomerInfo cst)
			throws UnsupportedEncodingException {
		String postData = null;
		if(message.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT)){
			postData = hi.getCommonMessageHandler().makeRequest(message, urlEncoding, cst);
		}else if(message.getMessageType().equals(GmmsMessage.MSG_TYPE_DELIVERY_REPORT)){
			postData = hi.getCommonDeliveryReportHandler().makeRequest(message, urlEncoding, cst);
		}
	...
	public String makeResponse(HttpStatus hs,GmmsMessage msg, A2PCustomerInfo cst) throws IOException, ServletException {
		...
		if(msg.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT) || msg.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT_RESP)){
			response = this.makeSubmitResponse(hs, msg, cst);
		}else if(msg.getMessageType().equals(GmmsMessage.MSG_TYPE_DELIVERY_REPORT) || msg.getMessageType().equals(GmmsMessage.MSG_TYPE_DELIVERY_REPORT_RESP)){
	 ...
	 */
	private String makeSubmitResponse(HttpStatus hs,GmmsMessage msg, A2PCustomerInfo cst) throws IOException, ServletException {
	 ...
	 * @return
	 */
	private String generateSubmitResponse(HttpStatus hs, GmmsMessage msg,HttpPdu submitResp, A2PCustomerInfo cst){
		...
		if(paramDelimiter==null || "".equals(paramDelimiter)){
			...
			for (HttpParam hp : respList) {
				...
				if("StatusText".equalsIgnoreCase(param)){
					content.append(oparam).append(paramDelimiter).append(hs.getText()).append(respDelimiter);
				}else if("StatusCode".equalsIgnoreCase(param)){
					content.append(oparam).append(paramDelimiter).append(hs.getCode()).append(respDelimiter);	
				}else{
					Object value = msg.getProperty(param);
					respValue = HttpUtils.getParameter(hp, msg, cst);
					respValue = respValue != null ? respValue : hp.getDefaultValue();
					...
					content.append(oparam).append(paramDelimiter).append(respValue).append(respDelimiter);
				}
				respValue = "";
			}
		}
		...
		if(respContent.endsWith(respDelimiter)){
			respContent = respContent.substring(0, respContent.length()-respDelimiter.length());
		}
		if(log.isDebugEnabled()){
	 ...
	 */
	private String makeDRResponse(HttpStatus hs, GmmsMessage message, A2PCustomerInfo cst) throws IOException, ServletException  {
		...
		if(paramDelimiter==null || "".equals(paramDelimiter)){
			...
			for (HttpParam hp : respList) {
				String param = hp.getParam();
				if ("StatusCode".equalsIgnoreCase(param)) {
					content.append(hp.getOppsiteParam()).append(paramDelimiter).append(hs.getCode()).append(respDelimiter);
				} else if ("StatusText".equalsIgnoreCase(param)) {
					content.append(hp.getOppsiteParam()).append(paramDelimiter).append(hs.getText()).append(respDelimiter);
				}else{
					respValue =  HttpUtils.getParameter(hp, message, cst);
					content.append(hp.getOppsiteParam() + paramDelimiter + respValue + respDelimiter);
				}
	 ...
	 * parse request
	 */
	public HttpStatus parseRequest(GmmsMessage message, HttpServletRequest request) throws ServletException, IOException {
		HttpStatus hs = null;
		if(message.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT)){
	 ...
	 * parse request
	 */
	private HttpStatus parseSubmitRequest(GmmsMessage message, HttpServletRequest request) throws ServletException, IOException {
		...
		if(hiUsername==null){
			...
			if (password == null || password.trim().length()<1) {
				message.setStatus(GmmsStatus.INVALID_MSG_FIELD);
				HttpStatus hs = hi.mapGmmsStatus2HttpSubStatus(GmmsStatus.INVALID_MSG_FIELD);				 
				return hs;
			}
		...
		try {
            ...
            log.warn("Error occur when processing throttling control in ExtendibleDelimiterHttpHandler.parseSubmitRequest",e);
        }
		...
		try {
			...
			String  commonMsgID = MessageIdGenerator.generateCommonMsgID(csts.getSSID());
	 ...
	 * parse DR request
	 */
	private HttpStatus parseDRRequest(GmmsMessage message,HttpServletRequest request) {
		try{
			...
			try {
	            ...
	            log.warn("Error occur when processing throttling control in ExtendibleDelimiterHttpHandler.parseDRRequest",e);
	        }
			...
			for (HttpParam hp : hi.getMtDRRequest().getParamList()) {
				String param = hp.getParam();
				String value = request.getParameter(hp.getOppsiteParam()) != null ? request.getParameter(hp.getOppsiteParam()) : hp.getDefaultValue();
				
					log.debug("param name={};OppsiteParam={}; requestvalue = {}; value={}",hp.getParam(),hp.getOppsiteParam(),request.getParameter(hp.getOppsiteParam()), value);
	...
	public void parseResponse(GmmsMessage message, String resp) {
		...
		if(message.getMessageType().equals(GmmsMessage.MSG_TYPE_SUBMIT)){
			...
			String[] lines = resp.split(respDelimiter);
			Map<String,String> respMap = genResponseMap(message,lines,paramDelimiter,false);
			for(HttpParam param:parameters){
				String pval = param.getParam();
				String oval = param.getOppsiteParam();
	 ...
	 * @throws Exception 
	 */
	private GmmsStatus genStatus4DR(String statusCode, String statusText) throws Exception {
		if(log.isDebugEnabled()){
