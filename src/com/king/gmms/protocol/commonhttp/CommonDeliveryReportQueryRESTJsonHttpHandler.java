...
public class CommonDeliveryReportQueryRESTJsonHttpHandler extends HttpHandler {

	private static SystemLogger log = SystemLogger
			.getSystemLogger(CommonDeliveryReportQueryRESTJsonHttpHandler.class);

	public CommonDeliveryReportQueryRESTJsonHttpHandler(HttpInterface hie) {
		super(hie);
	}
...
//				String encryptPass = HttpUtils.encrypt(sInfo.getChlPassword(),
//						encryptMethod);
...
//	}
	...
	private String generateResponse(HttpStatus hs, GmmsMessage msg,
		...
		try {
			...
			for (HttpParam hp : respList) {
				...
				ingredients.put(hp.getOppsiteParam(), respValue);
				respValue = null;
			}

			jsonObj.put("deliveryReport", ingredients);
	...
	public HttpStatus parseRequest(GmmsMessage msg, HttpServletRequest request)
		...
		try {
			...
			if ((outUsernameElt != null && outUsernameElt.trim().length() > 1)&&(outPasswordElt!=null&&outPasswordElt.trim().length()>1)) {
				username = request.getParameter(outUsernameElt);
				password = request.getParameter(outPasswordElt);
			} else if ((inUsernameElt != null && inUsernameElt.trim().length() > 1)&&(inPasswordElt!=null&&inPasswordElt.trim().length()>1)) {
