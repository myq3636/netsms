...
public class SIOOMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(SIOOMessageHttpHandler.class);

	public SIOOMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		String sign = HttpUtils.encrypt(chlAccount+chlPwd, "MD5");
		postData.append("&uid=").append(urlEncode(userId, mtype));
		postData.append("&auth=").append(urlEncode(sign, mtype));
		postData.append("&expid=0");
	...
	public void parseResponse(GmmsMessage msg, String resp) {
        ...
        if (respStr!= null && respStr.contains(",")) {
        	msg_status_code = respStr.split(",")[0];
        	outMsgID= respStr.split(",")[1];
		}               
