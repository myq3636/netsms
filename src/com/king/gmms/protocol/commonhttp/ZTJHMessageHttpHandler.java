...
public class ZTJHMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(ZTJHMessageHttpHandler.class);	

	public ZTJHMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		for (HttpParam param : parameters) {
			...
			if ("senderAddress".equalsIgnoreCase(pval)) {
				...
				msg.put(param.getOppsiteParam(), message.getTextContent());
			} else if ("recipientAddress".equalsIgnoreCase(pval)) {
				msg.put(param.getOppsiteParam(), recipient);
				...
				Password = HttpUtils.md5Encrypt(chpwd);
				long timestamp = System.currentTimeMillis();
				map.put("timestamp", timestamp+"");
				String md5Src = new StringBuffer(userName).append(Password)
						.append(timestamp).toString();
	    		if(log.isDebugEnabled()){
	    			log.debug("pssword md5 src: {}", md5Src);
	    		}
				String md5Pass = HttpUtils.md5Encrypt(md5Src);
				map.put("sign", md5Pass);
			} else {
	...
	public void parseResponse(GmmsMessage msg, String resp) {
		...
		if (map!=null && map.get("detail")!=null) {
			List<Map<String, String>> temp = (List<Map<String, String>>)map.get("detail");
