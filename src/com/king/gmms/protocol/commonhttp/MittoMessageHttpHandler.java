...
public class MittoMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(MittoMessageHttpHandler.class);	

	public MittoMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		if(GmmsMessage.AIC_CS_ASCII.equals(contentType)) {
			map.put("type", "GSM");
		}else if(GmmsMessage.AIC_CS_UCS2.equals(contentType)){
			map.put("type", "Unicode");
		}
	...
	public void parseResponse(GmmsMessage msg, String resp) {
        ...
        log.info("receive mitto submit response {}", resp);
		Map<String, Object> map = (Map<String, Object>)JSONObject.parse(respStr);
		String statusCode = "";
		if (map!=null && map.get("responseCode")!=null) {
			statusCode = String.valueOf(map.get("responseCode"));
			
		}	
		Object mittiId = map.get("id");
		if(mittiId!=null){
			String outmsgid = String.valueOf(mittiId);
			if(log.isDebugEnabled()){
				log.debug(msg, "outMsgId={}" , outmsgid);
			}
			msg.setOutMsgID(outmsgid);
		}else{
			log.warn("receive mitto submit response results is null");
		}
