...
import com.king.gmms.protocol.smpp.pdu.ByteData;
...
public abstract class TLV extends ByteData {
     ...
     *
     * @throws TLVException if the buffer contains somehow invalid data
     */
    protected abstract void setValueData(SmppByteBuffer buffer)
            throws TLVException;
     ...
     *
     * @throws ValueNotSetException if the TLV wasn't set any data
     *                              then there is nothing to be returned (the optional parameter
     *                              isn't transmitted then.)
     */
    protected abstract SmppByteBuffer getValueData()
    ...
    public int getLength()
            throws ValueNotSetException {
        if(hasValue()) {
            SmppByteBuffer valueBuf = getValueData();
            if(valueBuf != null) {
    ...
    public void setData(SmppByteBuffer buffer)
        ...
        SmppByteBuffer valueBuf = buffer.removeBuffer(length);
    ...
    public SmppByteBuffer getData()
            throws ValueNotSetException {
        if(hasValue()) {
            SmppByteBuffer tlvBuf = new SmppByteBuffer();
     ...
     */
    protected static void checkLength(int min, int max, SmppByteBuffer buffer)
            throws WrongLengthException {
        int length;
        if(buffer != null) {
     ...
     */
    protected void checkLength(SmppByteBuffer buffer)
            throws WrongLengthException {
        int length;
        if(buffer != null) {
            length = buffer.length();
