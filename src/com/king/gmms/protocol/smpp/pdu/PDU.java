...
public abstract class PDU extends ByteData {
    ...
    private static final int MAX_PDU_SIZE = 8*1024;
     ...
     * @see #setExtraOptional(short,SmppByteBuffer)
    ...
    // add for smpp version control by Jesse Duan@King.com

    public SMPPVersion version = SMPPVersion.getDefaultVersion();
     ...
     * @see #setHeader(SmppByteBuffer)
     * @see #setBody(SmppByteBuffer)
     * @see #setOptionalBody(SmppByteBuffer)
     * @see #getValid()
     * @see #getData()
     */
...
//                }
            ...
            SmppByteBuffer headerBuf = buffer.removeBytes(Data.PDU_HEADER_SIZE);
//            if(debug.active(DPDU)) {
...
//            }
            // parse the body
            if (Data.PDU_HEADER_SIZE < getCommandLength()){ //the body is not null
                setBody(buffer);
                setValid(VALID_BODY);
    ...
    public SmppByteBuffer getData()
            throws ValueNotSetException {
        // prepare all body
        SmppByteBuffer bodyBuf = new SmppByteBuffer();
        ...
        SmppByteBuffer pduBuf = getHeader();
...
//        }
     ...
     * @see PDUHeader#setData(SmppByteBuffer)
     */
    private void setHeader(SmppByteBuffer buffer)
            throws NotEnoughDataInByteBufferException {
        checkHeader();
     ...
     */
    private SmppByteBuffer getHeader() {
        checkHeader();
     ...
     * @throws UnexpectedOptionalParameterException
     *          if the optional
     ...
     * @see TLV
     */
    private void setOptionalBody(SmppByteBuffer buffer)
            throws NotEnoughDataInByteBufferException,
                   UnexpectedOptionalParameterException,
                   TLVException {
        short tag;
        short length;
        SmppByteBuffer tlvHeader;
        SmppByteBuffer tlvBuf;
        TLV tlv = null;
     ...
     * @see TLV
     */
    private SmppByteBuffer getOptionalBody()
            throws ValueNotSetException {
        SmppByteBuffer optBody = new SmppByteBuffer();
     ...
     * @see TLV
     */
    private SmppByteBuffer getOptionalBody(Vector optionalParameters)
            throws ValueNotSetException {
        SmppByteBuffer optBody = new SmppByteBuffer();
     ...
     */
    public void setExtraOptional(short tag, SmppByteBuffer data)
            throws TLVException {
     ...
     * @see com.king.gmms.protocol.smpp.Session#checkResponse(PDU,Response)
     */
     ...
     * @see #createPDU(int)
     */
    public static final PDU createPDU(SmppByteBuffer buffer)
        ...
        if(header.getCommandLength() > MAX_PDU_SIZE){
            try {
                buffer.removeBytes0(buffer.length());
            }
        ...
        if(buffer.length() < header.getCommandLength()) {
            ...
            // JmsReceiver must wait for more data
        ...
        try {
            ...
            if(pdu != null) {
                pdu.setData(buffer);
//                 if (pdu.getCommandId() == Data.BIND_RECEIVER ||
//                       pdu.getCommandId() == Data.BIND_TRANSCEIVER ||
//                       pdu.getCommandId() == Data.BIND_TRANSMITTER)
//                       if ( ( (BindRequest) pdu).getInterfaceVersion() >=
//                           0){
//                           version = SMPPVersion.getVersion( ( (
//                               BindRequest) pdu).getInterfaceVersion());
//                       }
                return pdu;
            }
            else {
    ...
    public static final void initPDU() {
        ...
        // add  for supporting smpp v3.3 by Jesse Duan@King.com
        pduList.add(new QueryLastMsgs());
        pduList.add(new QueryLastMsgsResp());
        pduList.add(new QueryMsgDetails());
        pduList.add(new QueryMsgDetailsResp());
        pduList.add(new ParamRetrieve());
        pduList.add(new ParamRetrieveResp());
        isInitialized = true;
     ...
     */
    public static final PDU createPDU(int commandId) throws VersionException {
        if(!isInitialized) {
            initPDU();
        }
...
/*
     static  {
        pduList = new Vector(30,4);
        System.out.println("PDU static begin to contruct ");
        pduList.add(new BindTransmitter());
        pduList.add(new BindTransmitterResp());
        pduList.add(new BindReceiver());
        pduList.add(new BindReceiverResp());
        ...
        pduList.add(new ParamRetrieve());
        pduList.add(new ParamRetrieveResp());
          System.out.println("PDU static end to contruct ");

    }
*/
    public String debugString() {
        StringBuilder dbgs = new StringBuilder().append("(pdu: ").append(super.debugString())
                .append(Integer.toString(getCommandLength())).append(" ")
                .append(Integer.toHexString(getCommandId())).append(" ")
                .append(Integer.toHexString(getCommandStatus())).append(" ");
        if(sequenceNumberChanged) {
            dbgs.append(getSequenceNumber());
        }
        else {
            // it's likely that this will be the assigned seq number
            // (in simple cases :-)
            dbgs.append("[").append(sequenceNumber + 1).append("]");
        }
        dbgs.append(") ");
        return dbgs.toString();
    }
    ...
    public void setVersion(SMPPVersion version) {
        if(version == null) {
            this.version = new SMPPVersion34();
        }
        else {
