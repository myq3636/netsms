...
import com.king.gmms.protocol.smpp.pdu.TerminatingZeroNotFoundException;
...
public class SmppByteBuffer extends SmppObject {

    private byte[] buffer;
    private static SystemLogger log = SystemLogger.getSystemLogger(SmppByteBuffer.class);
    ...
    public SmppByteBuffer(byte[] buffer) {
        this.buffer = buffer;
    }
    ...
    public void appendBuffer(SmppByteBuffer buf) {
        if(buf != null) {
            try {
    ...
    public void appendBytes(SmppByteBuffer bytes, int count)
    ...
    public SmppByteBuffer removeBuffer(int count)
            throws NotEnoughDataInByteBufferException {
        return removeBytes(count);
    }

    public SmppByteBuffer removeBytes(int count)
            throws NotEnoughDataInByteBufferException {
    ...
    public SmppByteBuffer readBytes(int count)
            throws NotEnoughDataInByteBufferException {
        int len = length();
        SmppByteBuffer result = null;
        if(count > 0) {
            if(len >= count) {
                byte[] resBuf = new byte[count];
                System.arraycopy(buffer, 0, resBuf, 0, count);
                result = new SmppByteBuffer(resBuf);
                return result;
            }
            else {
