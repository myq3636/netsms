...
public class AcceletClient extends AbstractClient {
	private static SystemLogger log = SystemLogger.getSystemLogger(ClickatellClient.class);
	private final String PROTOCOL_NAME = "ACCELET";
    private InternalAgentConnectionFactory agentFactory = null;
    private AcceletClientFactory customerFactory = null;
    
	public AcceletClient(){
		 try {
			 customerFactory = AcceletClientFactory.getInstance();
	            ...
	            if (alSsid != null && alSsid.size() > 0) {
	                ...
	                for (int i = 0; i < alSsid.size(); i++) {
	                    ...
	                    ci = gmmsUtility.getCustomerManager().getCustomerBySSID(
	                        ssid);
                        if (ctm.inCurrentA2P(ctm.getConnectedRelay(ssid,GmmsMessage.AIC_MSG_TYPE_TEXT))) {
