 ...
 * @version 1.0
 */
public class NodeConnectionManagerWithBindMode
    extends Observable implements NodeConnectionManagerInterface{

//    private Map<TransactionURI, Session> transactionURI2Connections;
    private ConnectionPool connectionPool = null;
    private IndexStrategy strategy = null;
    private Object mutex = new Object();
    private Node node = null;

    public NodeConnectionManagerWithBindMode() {
        this(new IndexRandomStrategy());
    }

    public NodeConnectionManagerWithBindMode(IndexStrategy strategy) {
    ...
    public Session getSession() {
        ...
        if(index < connectionPool.getAvailableSize()){
            for(int i = index; i < connectionPool.getAvailableSize(); i++){
                Connection connection = connectionPool.getAvailableConnection(i);
                if(connection != null && (connection.getBindMode().equals(BindMode.Transmitter)
                    || connection.getBindMode().equals(BindMode.Transceiver))){
    ...
    public Session getSession(ArrayList connectionIDList,boolean isSplit4DR) {
        ...
        while (iterator.hasNext()) {
            ...
            if (obj != null) {
                ...
                if (connection != null && isSplit4DR &&
                    connection.getBindMode().equals(BindMode.Receiver)) {
                    String tn = (String) obj;
                    tn = tn.substring(0, tn.length() - 2);
                    connection = connectionPool.getAvailableConnection(tn);
                    if (connection != null)
                        return connection.getSession();
                }
            }
