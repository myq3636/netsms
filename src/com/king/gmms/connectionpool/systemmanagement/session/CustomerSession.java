...
public class CustomerSession extends Session {
	...
	private static SystemLogger log = SystemLogger
			.getSystemLogger(CustomerSession.class);
	...
	public CustomerSession(TransactionURI transaction, boolean isServer) {
	...
	public void stop() {
		...
		case CONNECT:
			if(log.isInfoEnabled()){
				log.info("The session({}) status is set:  DISCONNECT and the original status is CONNECT",
							getSessionName());
			}
		...
		case INITIAL:
			if(log.isInfoEnabled()){
				log.info("The session({}) status is set:  DISCONNECT and the original status is INITIAL",
							getSessionName());
			}
			setStatus(ConnectionStatus.DISCONNECT);
			break;
